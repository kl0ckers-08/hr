...
import { LayoutDashboard, CalendarDays, CalendarX, Clock, FileBarChart, LogOut, X, Menu } from 'lucide-react';
        ...
        <>
            ...
            <aside className={`sidebar ${isMobileOpen ? 'mobile-open' : ''}`}>
                ...
                <nav className="sidebar-nav">
                    {NAV_ITEMS.map((item) => (
                        <NavLink key={item.path} to={item.path} className={({ isActive }) => `nav-item ${isActive ? 'active' : ''}`} end={item.path === '/dean'} onClick={() => setIsMobileOpen(false)}>
